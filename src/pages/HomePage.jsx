...
import { Shield, Users, Target, Code, Sparkles } from 'lucide-react';
...
const SyllabusWeek = ({ week, accentColor }) => {
    ...
    const handleExplain = async () => {
        ...
        setExplanation(result); setIsLoading(false);
    };
    return (
        <div className="relative">
            ...
            <button onClick={handleExplain} disabled={isLoading} className={`mt-2 text-sm text-${accentColor}-400 hover:text-${accentColor}-300 flex items-center gap-1 disabled:text-slate-500`}><Sparkles size={14} />{isLoading ? 'Explaining...' : '✨ Explain Topic'}</button>
