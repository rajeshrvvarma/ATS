...
import { Shield, Users, Target, Code, Sparkles, X, Briefcase, UserCheck, MonitorPlay, CheckSquare } from 'lucide-react';
...
const About = () => ( <section id="about" className="py-20 bg-slate-800"><div className="container mx-auto px-6"><SectionTitle>About Agnidhra Technologies</SectionTitle><div className="max-w-4xl mx-auto text-center"><p className="text-lg text-slate-300">In today's rapidly evolving tech landscape, theoretical knowledge is not enough. Agnidhra Technologies was founded on the principle of bridging the critical gap between academic learning and real-world industry demands. Our mission is to empower aspiring tech professionals with practical, hands-on skills that make them not just knowledgeable, but job-ready from day one.</p></div></div></section>);

const WhyUs = () => {
    const points = [
        { icon: Briefcase, title: "Project-Based Learning", description: "You don't just learn theory; you build it. Our curriculum is centered around hands-on labs and a capstone project that builds a portfolio you can proudly showcase to employers." },
        { icon: UserCheck, title: "Expert Mentorship", description: "Benefit from personalized guidance from industry veterans with direct, multi-domain experience in both offensive and defensive security operations." },
        { icon: CheckSquare, title: "Career-Focused", description: "We constantly update our courses to align with the latest industry trends and teach you the exact skills employers are looking for in SOC Analysts and Ethical Hackers right now." },
        { icon: MonitorPlay, title: "Flexible Online Learning", description: "Learn from anywhere. Our live, interactive online classes are designed to fit your schedule, and all sessions are recorded for your convenience." }
    ];

    return (
        <section id="whyus" className="py-20 bg-slate-900">
            <div className="container mx-auto px-6">
                    ...
                    {points.map((point, index) => (
                        <div key={index} className="text-center">
                            <div className="flex justify-center items-center mb-4">
                                <point.icon className="w-12 h-12 text-sky-400" />
                            </div>
...
const Admissions = () => (
    <section id="admissions" className="py-20 bg-slate-800">
        <div className="container mx-auto px-6">
            ...
            <div className="max-w-4xl mx-auto flex flex-col md:flex-row justify-center items-center gap-4 md:gap-8">
                ...
                <div className="text-center flex-1">
                    <div className="w-16 h-16 rounded-full bg-sky-500 flex items-center justify-center text-2xl font-bold text-white mx-auto mb-4 border-4 border-slate-700">1</div>
                    <h3 className="text-xl font-bold text-white mb-2">Submit an Inquiry</h3>
                    <p className="text-slate-400">Fill out our contact form with your details and course of interest.</p>
                </div>
                <div className="text-sky-400 hidden md:block text-4xl font-light">›</div>
                {/* Step 2 */}
                <div className="text-center flex-1 mt-8 md:mt-0">
                     <div className="w-16 h-16 rounded-full bg-sky-500 flex items-center justify-center text-2xl font-bold text-white mx-auto mb-4 border-4 border-slate-700">2</div>
                ...
                {/* Step 3 */}
                <div className="text-center flex-1 mt-8 md:mt-0">
                     <div className="w-16 h-16 rounded-full bg-sky-500 flex items-center justify-center text-2xl font-bold text-white mx-auto mb-4 border-4 border-slate-700">3</div>
...
const CourseOfferings = ({ setVisibleSyllabus, onNavigate }) => {
    ...
    return ( <section id="offerings" className="py-20 bg-slate-900"><div className="container mx-auto px-6"><SectionTitle>Choose Your Cyber Security Path</SectionTitle><div className="max-w-xl mx-auto mb-16"><div className="bg-slate-800 border border-slate-700 rounded-lg p-8 flex flex-col text-center items-center hover:border-slate-500 transition-all duration-300 transform hover:-translate-y-2"><freeWorkshop.icon size={32} className="mb-4 text-sky-400" /><h3 className="text-2xl font-bold text-white mb-3">{freeWorkshop.title}</h3><p className="text-slate-400 mb-6">{freeWorkshop.description}</p><button onClick={() => onNavigate(freeWorkshop.page)} className="w-full max-w-xs mt-auto bg-sky-500 text-white font-semibold py-3 rounded-lg hover:bg-sky-600 transition-colors duration-300">{freeWorkshop.cta}</button></div></div><div className="grid md:grid-cols-2 gap-12 max-w-6xl mx-auto"><div className="flex flex-col space-y-8"><div className="flex items-center gap-4"><defensivePath.icon size={40} className="text-sky-400" /><h3 className="text-3xl font-bold text-white">{defensivePath.title}</h3></div>{defensivePath.courses.map((course, index) => ( <div key={index} className="bg-slate-800 border border-slate-700 rounded-lg p-6 flex flex-col hover:border-sky-500 transition-all duration-300 transform hover:-translate-y-1 h-full"><h4 className="text-2xl font-bold text-white mb-3">{course.title}</h4><p className="text-slate-400 mb-6 flex-grow">{course.description}</p><ul className="space-y-2 mb-8 text-slate-300">{course.features.map((feature, i) => <li key={i} className="flex items-center"><Target size={16} className="text-sky-400 mr-3 flex-shrink-0" /><span>{feature}</span></li>)}</ul><button onClick={() => course.syllabus ? handleSyllabusClick(course.syllabus) : onNavigate(course.page)} className="w-full mt-auto bg-sky-600 text-white font-semibold py-3 rounded-lg hover:bg-sky-700 transition-colors duration-300">{course.cta}</button></div>))}</div><div className="flex flex-col space-y-8"><div className="flex items-center gap-4"><offensivePath.icon size={40} className="text-red-400" /><h3 className="text-3xl font-bold text-white">{offensivePath.title}</h3></div>{offensivePath.courses.map((course, index) => ( <div key={index} className="bg-slate-800 border border-slate-700 rounded-lg p-6 flex flex-col hover:border-red-500 transition-all duration-300 transform hover:-translate-y-1 h-full"><h4 className="text-2xl font-bold text-white mb-3">{course.title}</h4><p className="text-slate-400 mb-6 flex-grow">{course.description}</p><ul className="space-y-2 mb-8 text-slate-300">{course.features.map((feature, i) => <li key={i} className="flex items-center"><Target size={16} className="text-red-400 mr-3 flex-shrink-0" /><span>{feature}</span></li>)}</ul><button onClick={() => course.syllabus ? handleSyllabusClick(course.syllabus) : onNavigate(course.page)} className="w-full mt-auto bg-red-600 text-white font-semibold py-3 rounded-lg hover:bg-red-700 transition-colors duration-300">{course.cta}</button></div>))}</div></div></div></section>)};
